...
function isTlsErrorMessage(msg, code) {
  ...
  const s = (msg || "") + " " + (code || "");
  return /certificate|ssl|tls|unable to get local issuer|unable to verify the first certificate|CERT_|ERR_TLS|DEPTH_ZERO_SELF_SIGNED_CERT/i.test(
    s,
  );
}
  ...
  try {
    ...
    if (context) {
      try {
        ...
        console.warn("context.close failed:", e && e.message ? e.message : e);
      }
    }
  }
}
...
async function run() {
  ...
  for (const file of ymlFiles) {
    ...
    if (doc && doc.site) {
      ...
      addUrl(doc.site.url, file, "main", {
        name: doc.site.name,
        parentName: doc.site.name,
        parentUrl: doc.site.url,
      });
      const friends = doc.site.friends || [];
      if (Array.isArray(friends)) {
        friends.forEach((f, idx) => {
          if (f && f.url)
            addUrl(f.url, file, "friend", {
              index: idx,
              name: f.name,
              parentName: doc.site.name,
              parentUrl: doc.site.url,
      ...
      (async () => {
        while (queue.length) {
          ...
          try {
            ...
            console.log(
              `  结果： ${res.ok ? "正常" : "不可达"}  ${res.status ? `status=${res.status}` : `err=${res.error}`}  time=${duration}ms`,
            );
            if (!res.ok) {
  ...
  try {
    ...
    console.warn("browser.close failed:", e && e.message ? e.message : e);
  }
